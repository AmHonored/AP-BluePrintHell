...
public class PacketStatistics {
    ...
    private void updateHudPacketCounter(int count) {
        if (hasGameScene()) {
            gameState.getGameScene().updatePacketsCollectedLabel(count);
        }
    }
    ...
    private void updateHudPacketCounterAsync() {
        if (hasGameScene()) {
            Platform.runLater(() -> {
                ...
                gameState.getGameScene().updatePacketsCollectedLabel(displayCount);
                System.out.println("HUD packet counter updated to " + visualPacketsDelivered);
            });
        }
    }
    ...
    private void updateHudPacketLossLabel() {
        if (hasGameScene()) {
            gameState.getGameScene().updatePacketLossLabel(gameState.getPacketLossPercentage());
        }
    }
    
    private boolean hasGameScene() {
        return gameState.getGameScene() != null;
    }
