...
public class VisualManager {
    // Constants
    private static final double INTERMEDIATE_WIDTH = 80;
    private static final double INTERMEDIATE_HEIGHT = 100;
    private static final double PADDING = 10;
    private static final double STANDARD_PACKET_SIZE = 12.0;
    private static final double LAMP_RADIUS = 5;
    private static final int MAX_CAPACITY = 5;
    private static final long VISIBILITY_UPDATE_INTERVAL = 83; // ~12 updates per second
    private static final long PACKET_UPDATE_INTERVAL = 250; // 1/4 second
    ...
    public VisualManager(NetworkSystem parentSystem, Point2D position, double width, double height) {
        ...
        // Use consistent dimensions for intermediate systems
        boolean isIntermediate = !parentSystem.isStartSystem() && !parentSystem.isEndSystem();
        double systemWidth = isIntermediate ? INTERMEDIATE_WIDTH : width;
        double systemHeight = isIntermediate ? INTERMEDIATE_HEIGHT : height;
        
        if (isIntermediate) {
            System.out.println("Forcing system dimensions to 80x100 for: " + parentSystem.getLabel());
        }
    ...
    public void rearrangeStoredPackets() {
        ...
        if (innerBox == null || packets.isEmpty()) {
            return;
        }
    ...
    public void positionPacketInInnerBox(Packet packet) {
        ...
        if (!parentSystem.isStartSystem() && !parentSystem.isEndSystem() && innerBox != null && !hasManuallySetPosition) {
    ...
    private boolean checkCorrectConnections() {
        if (parentSystem.isStartSystem()) {
            ...
            return !parentSystem.getOutputPorts().isEmpty() && 
                   parentSystem.areAllPortsCorrectlyConnected();
        } else if (parentSystem.isEndSystem()) {
            // End system only needs input ports connected
            return !parentSystem.getInputPorts().isEmpty() && 
                   parentSystem.areAllPortsCorrectlyConnected(); 
        } else {
            // Regular system needs both input and output ports connected
            return !parentSystem.getInputPorts().isEmpty() && 
                   !parentSystem.getOutputPorts().isEmpty() && 
                   parentSystem.areAllPortsCorrectlyConnected();
        }
    }
