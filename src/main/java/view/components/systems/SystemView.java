...
public abstract class SystemView extends StackPane {
    ...
    /**
     * Create a port view based on port type
     */
    private PortView createPortView(Port port) {
        boolean isInput = port.getType() == model.entity.ports.PortType.INPUT;
        if (port instanceof model.entity.ports.SquarePort) {
            return new view.components.ports.SquarePortView(port, isInput);
        } else if (port instanceof model.entity.ports.TrianglePort) {
            return new view.components.ports.TrianglePortView(port, isInput);
        } else {
